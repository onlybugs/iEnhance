...
class MultiscaleSpatialProjection(nn.Module):
    def __init__(self,big = 64,small = 32) -> None:
        super(MultiscaleSpatialProjection,self).__init__()
...
class ResBlock(nn.Module):
    def __init__(self,num_layers,in_channel,out_channel,ks = 1,p = 0,s = 1):
        ...
        for i in range(num_layers):
            if(i == 0):
                self.upfeature = RA(in_channel,out_channel,ks,p,True,s)
            else:
                simple_block.append(('res'+str(i+1),RA(out_channel,out_channel,ks,p,False,s)))
...
class iEnhance(nn.Module):
    def __init__(self) -> None:
        ...
        self.mix = MultiscaleSpatialProjection()
        self.den = Dencoder()
        self.res = ResBlock()
